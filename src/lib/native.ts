// Integrasi Capacitor (Android/iOS): status bar, splash, tombol back Android. Di web semua no-op.
...
export async function initNative(onBack: () => boolean) {
  ...
  try {
    ...
    if (platform === "android") await StatusBar.setBackgroundColor({ color: "#FFFFFF" });
  } catch {
